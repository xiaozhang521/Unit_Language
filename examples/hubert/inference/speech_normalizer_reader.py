...
class SpeechNormalizerReader:
    """
    Wrapper class to run inference on HuBERT model.
    Helps extract features for a given audio file.
    """

    def __init__(self, checkpoint_path, max_chunk=1600000, use_cuda=True):
        (
            model,
            cfg,
            task,
    ...
    def get_feats(self, file_path, ref_len=None, channel_id=None):
        ...
        with torch.no_grad():
            ...
            for start in range(0, x.size(1), self.max_chunk):
                ...
                feat_chunk = self.model(source=x_chunk, padding_mask=None) 
                feat.append(feat_chunk['encoder_out'])
                
            logits = torch.cat(feat, 0).squeeze(1)
            probabilities = F.softmax(logits, dim=-1)
            predicted_index = (torch.argmax(probabilities, dim=-1).reshape(-1) - 4)
            index = predicted_index >= 0
            predicted = predicted_index[index]
            return predicted
