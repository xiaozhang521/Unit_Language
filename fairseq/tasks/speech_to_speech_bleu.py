...
@register_task("speech_to_speech_bleu")
class SpeechToSpeechBLEUTask(LegacyFairseqTask):
    @classmethod
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        if self.args.eval_inference or self.eval_bleu:
    ...
    def train_step(
        ...
        # separate_task = None # ["target_text"]
        
        # if separate_task is None:
        #     loss, sample_size, logging_output = super().train_step(
        #         sample, model, criterion, optimizer, update_num, ignore_grad
        #     )
        # else:
        #     # separate the specific multi-task loss
        #     # model._set_static_graph()
        #     model.train()
        #     model.set_num_updates(update_num)
        #     with torch.autograd.profiler.record_function("forward"):
        #         with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
        #             loss, loss_separated, sample_size, logging_output = criterion(model, sample, separate_task=separate_task)
        #             # print("loss_separated: ", loss_separated)
            
        #     if ignore_grad:
        #         loss *= 0
        #         for task in loss_separated.keys():
        #             loss_separated[task] *= 0
        ...
        #     #     if "encoder.subsample" in name:
        #     #         param.requires_grad = False
        #     #         freeze_list.append(name)
        #     #     else:
        #     #         for i in range(unit_layers + 1):
        #     #             if f"encoder.transformer_layers.{i}." in name:
        #     #                 param.requires_grad = False
        #     #                 freeze_list.append(name)
        #     #                 break
        #     # with torch.autograd.profiler.record_function("backward"):
        #     # for task in loss_separated.keys():
        #         # optimizer.backward(loss_separated[task])
        #         # loss_separated[task].backward(retain_graph=True)
            
        #     for task in loss_separated.keys():
        #         loss_sep += loss_separated[task]
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.eval_bleu:
            bleu = self._inference_with_bleu(self.generator, sample, model, hypos)
    ...
    def _inference_with_bleu(self, generator, sample, model, gen_out=None):
        import sacrebleu
