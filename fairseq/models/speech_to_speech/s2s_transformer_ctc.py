...
class S2STransformerMultitaskModelBase(FairseqEncoderDecoderModel):
    ...
    def build_multitask_decoder(cls, args, tgt_dict, in_dim):
        ...
        if args.decoder_type == "transformer":
            ...
            base_multitask_transformer_decoder_arch_textless(decoder_args)
    ...
    def build_model(cls, args, task):
        ...
        for task_name, task_obj in task.multitask_tasks.items():
            print(f"{task_name}: {task_obj.args.input_layer}")
            in_dim = (
...
@register_model("s2ut_transformer_ctc")
class S2UTTransformerModelCTC(S2STransformerMultitaskModelBase):
    """
    ...
    def forward(
        ...
        if "inner_ctc" in self.multitask_decoders.keys():
            layer_id = "12"
            ctc_model = multitask_decoders["inner_ctc"]
            
            if len(encoder_out["encoder_padding_mask"]) > 0:
                non_padding_mask = ~encoder_out["encoder_padding_mask"][0]
                input_lengths = non_padding_mask.long().sum(-1)
            else:
                out = encoder_out["encoder_states"][layer_id]
            ...
            assert encoder_out["encoder_out"] == encoder_out["encoder_states"][layer_id]

            seq_len, bsz, _ = ctc_input["net_input"]["src_tokens"].size()
            
            probs = ctc_model(**ctc_input["net_input"])["encoder_out"]
            vocab_size = probs.size(-1)
            
            print(ctc_input["net_input"]["src_tokens"].size(), probs.size(), self.decoder.output_projection.weight.size())
            assert False
            
            # distribution = ctc_model.get_normalized_probs(probs, log_probs=True).contiguous()  # (T, B, C) from the encoder
            distribution = F.softmax(probs / ctc_model.distribution_temperature, dim=-1)
            
            distribution_2d = distribution.contiguous().view(-1, vocab_size)
            soft_out = torch.mm(distribution_2d, self.decoder.output_projection.weight).view(seq_len, bsz, -1)
            
            linear_out = ctc_model.linear_adapter(encoder_out["encoder_out"])
            encoder_out["encoder_out"] = ctc_model.out_ln(linear_out + soft_out)
...
def base_multitask_transformer_decoder_arch_textless(args):
    args.dropout = getattr(args, "dropout", 0.3)
    args.decoder_layerdrop = getattr(args, "decoder_layerdrop", 0.0)
    args.share_decoder_input_output_embed = getattr(
        args, "share_decoder_input_output_embed", True
    ...
    args.max_target_positions = getattr(args, "max_target_positions", 3000)
    args.no_scale_embedding = getattr(args, "no_scale_embedding", False)
...
@register_model_architecture(
    model_name="s2ut_transformer_ctc", arch_name="s2ut_transformer_ctc"
)
...
@register_model_architecture("s2ut_transformer_ctc", "s2ut_transformer_ctc_textless")
def s2ut_transformer_textless(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.dropout = getattr(args, "dropout", 0.1)
