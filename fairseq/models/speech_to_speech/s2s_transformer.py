...
class S2STransformerMultitaskModelBase(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        setattr(base_model.encoder, "inner_ctc", {})
        setattr(base_model.encoder, "inner_ctc_layer", {})
        for task_name, task_obj in task.multitask_tasks.items():
            ...
            if "inner_ctc" in task_name.split('_', 1):  # source_inner_ctc / target_inner_ctc
                
                base_model.encoder.inner_ctc_layer[int(task_obj.args.input_layer)] = task_name
                base_model.encoder.inner_ctc[task_name] = [base_model.multitask_decoders[task_name]]
                base_model.encoder.inner_ctc[task_name][0].encoder.output_projection = base_model.decoder.output_projection
                
        print("Inner_ctc: {}".format(base_model.encoder.inner_ctc_layer))
        return base_model
...
class S2UTTransformerModel(S2STransformerMultitaskModelBase):
    ...
    def forward(
        ...
        if return_all_hiddens:
            ...
            decoder_out[-1]["old_padding_mask"] = encoder_out[
                "old_padding_mask"
            ]
        return decoder_out
...
@register_model_architecture("s2ut_transformer", "s2ut_transformer_textless")
...
@register_model_architecture("s2ut_transformer", "s2ut_transformer_textless_shrink")
def s2ut_transformer_textless(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.dropout = getattr(args, "dropout", 0.1)
    args.use_ctc_shrink = True
    s2ut_architecture_base(args)
    
@register_model_architecture("s2ut_transformer", "s2ut_transformer_textless_prefix")
def s2ut_transformer_textless(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.dropout = getattr(args, "dropout", 0.1)
    args.use_prefix_token = True
    args.add_prefix_layer = "6,8"
    s2ut_architecture_base(args)
