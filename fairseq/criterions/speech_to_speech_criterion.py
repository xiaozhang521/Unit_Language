...
class MultitaskCriterion:
    ...
    def get_multitask_loss(self, model, sample, model_out):         # , separate_task=[]
        # loss_separated = {}
        logging_output = {}
        loss = 0.0
        for task_name, task_criterion in self.multitask_criterion.items():

            if task_name in ["source_unit"]:
                # self.multitask_loss_weight[task_name] = 0.0
                # for (name, param) in model.multitask_decoders[task_name].named_parameters():
            ...
            if isinstance(task_criterion, CtcCriterion):
                if task_criterion.task.args.input_from == "encoder":
                    #if len(model_out["encoder_padding_mask"]) > 0:
                    if len(model_out["old_padding_mask"]) > 0:
                        non_padding_mask = ~model_out["old_padding_mask"][0]
                        input_lengths = non_padding_mask.long().sum(-1)
                    else:
                        out = model_out["encoder_states"][layer_id]
                        input_lengths = out.new_full(
                ...
                if task_name in ["source_unit"] or (model.encoder.use_prefix_token and layer_id <= min(model.encoder.add_prefix_layer)):  # or (model.encoder.use_prefix_token and layer_id <= min(model.encoder.add_prefix_layer))   # , "source_text"
                    task_sample = {
                        "net_input": {
                            "src_tokens": sample["multitask"][task_name]["net_input"][
                                "prev_output_tokens"
                            ...
                            "encoder_out": {
                                "encoder_out": [model_out["encoder_states"][layer_id]],
                                "encoder_padding_mask": model_out["old_padding_mask"],
                            },
                        }
                    }
                # elif task_name in separate_task:
                #     # re-forward
                #     encoder_states = []
                #     unit_layer = 5
                #     x = model_out["attention_in"]
                #     for i, layer in enumerate(model.encoder.transformer_layers):
                #         x = layer(x, model_out["attention_mask"])
                #         if i == unit_layer:
                #             x = x.detach()
                ...
                #             ],
                #             "encoder_out": {
                #                 # "encoder_out": [extra["encoder_states"][layer_id]],
                #                 # "encoder_padding_mask": extra["encoder_padding_mask"],
                #                 "encoder_out": [encoder_states[layer_id]],
                #                 "encoder_padding_mask": model_out["encoder_padding_mask"],
                #             },
                #         }
                #     }
            ...
            loss = loss + self.multitask_loss_weight[task_name] * task_loss
            # if task_name in separate_task:
            #     loss_separated[task_name] = self.multitask_loss_weight[task_name] * task_loss
            # else:
            #     loss = loss + self.multitask_loss_weight[task_name] * task_loss
            task_logging_output["loss_weight"] = self.multitask_loss_weight[task_name]
            logging_output[task_name] = task_logging_output
...
class SpeechToUnitMultitaskTaskCriterion(
    ...
    def forward(self, model, sample, reduce=True, separate_task=[]):
        net_input_concat = {
            "src_tokens": sample["net_input"]["src_tokens"],
            "src_lengths": sample["net_input"]["src_lengths"],
            "prev_output_tokens": sample["net_input"]["prev_output_tokens"],
        ...
        # prefix tokens
        if model.encoder.pre_tokens is not None and model.encoder.use_prefix_token and model.encoder.both_activate:
            prefix_loss_scale = 1.0
            prefix_loss = (model.encoder.pre_tokens[0] - model.encoder.pre_tokens[1]).norm() * -1.0 * prefix_loss_scale
            loss += prefix_loss
            logging_output["prefix_loss"] = utils.item(prefix_loss.data)
            if model.num_updates % 500 == 1:
                print("prefix_loss:", prefix_loss, model.encoder.pre_tokens.requires_grad)  # , model.encoder.pre_tokens[0], model.encoder.pre_tokens[1]

        return loss, sample_size, logging_output
        # if len(separate_task) == 0:
        #     return loss, sample_size, logging_output
        # else:
        #     return loss, loss_separated, sample_size, logging_output
